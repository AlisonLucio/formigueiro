...
def test_create_dags_list(dags_directory: str) -> None:
    ...
    assert f"{temp_dir}/api_compras_dp.py" in dags
    assert f"{temp_dir}/combustiveis_auto_br.py" in dags
    assert f"{temp_dir}/tests/api_compras_dp_test.py" not in dags
    assert f"{temp_dir}/tests/combustiveis_auto_br.py" not in dags
...
def test_upload_dags_to_composer(
    ...
    add_dags_to_composer.upload_dags_to_composer(DAGS_DIR, test_bucket, "../dags/tests/")
    out, _ = capsys.readouterr()
